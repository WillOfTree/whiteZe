...
+ 用于计算变量大小（单位：字节B），sizeof是运算符，不是函数
...
// 这里a是一个指针，指向数组{1,2,3}
sizeof(a); //12B, 计算整个数组大小（3*4=16）

// 这里a[0]指向的是变量1，变量1是int类型，4B
sizeof(a[0]); //4B，int类型4字节

//a=a+1不参与其他运算，在编译时已经完成计算
sizeof(a = a + 1);
```
...
{
    printf(“%d”, a[0]); // 数组a[0]，int类型4B
    printf(“%d”, sizeof(a)); //这里的a是指针，指针大小（32位4B，64位8B ）
}

int main()
{
    int a[] = {1,2,3}
    int b = sizeof(a); //a是一个指针，指向{1,2,3}，计算整体大小
    func_02(a, 3);
}
...
void func_01(stu *s);
{
    ...
    // 错误计算：
    // 这样只能计算整个数组的个数，不能计算一共有几个有效数据大小
    // int a = sizeof(s->aa)/sizeof(s->aa[0]);
    //
}
...
- `const int *ptr = &var;` 等价于`int const *ptr = &var`
...
常量引用：`const int & ref = var` 

``` c++
const int & ref = var; // ref只能读取
ref = 20; // 错误，不可赋值，只能读取
```
...
- 用途1：整个数组在内存中占用的空间的大小，
- 用途2：获取数组在内存中的首地址
- 用途3：数组名增加减少，等于移动指针在数组上的位置

``` c
int arr[5] = {1,2,3,4,5};
sizeof(arr); // 统计整个数组长度，5*4=20B，int类型4B
...
const int *p = &a
// 例子
*p = 20; //const int 所以p指向的值不能修改，*p不能修改
p = &b;  //可以修改 p的值 
```
...
1. 当只用来保存数据，没有复杂行为时，使用结构体，

2. 结构体与Class是等价的，Class默认权限是private，而结构体是public

3. 结构体可以相互赋值（相同结构），结构体数组不能相互赋值
...
6. 关于大结构体变量优先使用指针
   ...
   - 使用指针：`std::array<my_struct *, 20> a;` 
   - 使用智能指针：`std::array<std::unique_ptr<my_struct>, 10> arr;` 
...
- 如果结构体仅包含基本类型（如 `int`、`double`）或其他自动管理资源的类型（如 `std::string`、`std::vector`），则 **不需要手动释放**，析构时会自动清理。
  ...
  std::array<SimpleStruct, 3> arr;
  // 直接覆盖元素即可
  arr[1] = SimpleStruct{};  // 无需释放
  ```

- 如果结构体包含**裸指针**、**文件句柄** 或**其他需要手动释放的资源**，视情况而定

  ``` c++
  struct SimpleStruct {
      Node2D * x = null; // 一般空置即可，若使用new，应对应使用delete
  };
  ```

- 如果结构体本身通过 `new` 分配，则需要 `delete`：
