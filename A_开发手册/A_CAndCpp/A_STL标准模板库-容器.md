...
- 容器：各种数据结构，vector、list、map等
    ...
    3. 无序关联容器：unordered_set、unordered_map、unordered_multisetset、unordered_multisetmap
...
1. vector：动态数组
   ...
   2. 使用deque
      ...
      需要头插/头删或实测 `deque` 更快时切换

      deque的随机访问效率略低于vector
...
- std::array不能直接存储引用：`std::array<BigStruct &, 2> arr; //错误` 

- 对于二维数组`Array[a][b]`，其中a是行，b是列

  `std::array<std::array<int, c>, d>`：d是行，c是列

- **普通数组**：
  - 无成员函数，访问越界时行为未定义（可能崩溃或内存污染）
  - 传递给函数时会退化为指针，丢失大小信息。
  
- std::array
  - 提供边界检查（`at()` 方法越界会抛出异常），更安全
...
``` c++
/* 一维数组 */
std::array<int, 5> myArray; // 定义
// 初始化为0，这两种方法性能上没有差别，
std::array<int, 5> myArray {}; // C++11新语法
std::array<int, 5> myArray = {};// 老语法

// 初始化为固定值
std::array<int, 5> myArray {1, 2, 3, 4, 5}; // 新语法
std::array<int, 5> myArray = {1, 2, 3, 4, 5}; // 全部定义
std::array<int, 5> myArray = {1, 2}; // 只定义前2个，其余为0
...
// 定义-分配内存
std::list<int> L {1, 2, 3};// 列表初始化
std::list<int> m1(m.begin()， m.end()); // 迭代器拷贝
std::list<int> m2(m); // 拷贝
std::list<int> m3(5, 1000); // 5个元素，每个1000
```
