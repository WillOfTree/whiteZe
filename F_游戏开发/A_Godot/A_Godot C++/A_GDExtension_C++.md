...
2. Ref<>定义的类需要完全的定义，不能使用向前声明，必须引入头文件
...
var obj = ClassDB.instantiate("MyCPPClass") as MyCPPClass
...
1. **bind_method返回的类型必须是Godot定义的类型**
2. 返回类型Godot会自动转换类型
3. 优先返回具体的类型，不要使用Variant定义所有的返回值，因为Variant多了一层检测
...
#### Ⅰ、必须使用bind_static_method

1. 根据游戏模式返回不同的单例实现
2. 权限校验失败时返回空或降级对象
3. 一般不使用bind_static_method，**暂时没遇见必须使用的情况**
4. `add_singleton` 只能注册 `Object` 派生类，而 `bind_static_method` 可以返回任意类型。
...
1. 继承自RefCounted不需要析构函数
2. 继承自Object需要析构函数
3. 一般来说，只在C++内部使用

GDScript调用

- 需要使用` ClassDB::bind_static_method`将`get_singleton`注册为静态方法才可使用
- 即使暴露给GDScript，也推荐使用“注册单例模式”
...
``` c++
/*  */ 
class TTrayExtension : public Object 
{
    GDCLASS(TTrayExtension, Object);
private:
    // 声明，对象的保存位置
    static TTrayExtension* singleton; 
...
``` c++
TTrayExtension* TTrayExtension::singleton = nullptr; // 定义 + 初始化
...
``` python
# 调用方法1
# 通过Engine方法获取单例，MySingleton是单例名称
